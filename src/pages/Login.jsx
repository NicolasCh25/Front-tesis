...
const Login = () => {
  ...
  // ⬇️ RENOMBRAMOS setRol para evitar conflicto
  ...
  const handleLogin = async (dataForm) => {
    ...
    if (response) {
      setToken(response.token);         // Guardamos token global
      setRolGlobal(response.rol);       // Guardamos rol global
      navigate("/dashboard");
    }
  };
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen py-8 overflow-y-auto bg-gray-100">
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-3xl shadow-xl p-8 mx-4 my-8 mt-24 border border-gray-200">
        ...
        {/* Google */}
        <button className="bg-white border border-gray-300 py-2 w-full rounded-full flex justify-center items-center text-sm text-gray-700 hover:scale-105 duration-300 hover:bg-gray-100">
          <img
